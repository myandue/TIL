...
- **비관적(or 보수적) 동시성 제어(PCC, Pessimistic Concurrency Control)**
  - 잠금 기반 방식
    - 각 트랜잭션은 다른 트랜잭션이 같은 레코드를 동시에 수정 및 접근할 수 없도록 레코드에 대한 잠금을 획득한다.
    - 여러 트랜잭션이 서로 잠금을 해제하기를 기다리는 교착상태(데드록)가 발생할 수 있다.
  - 무잠금 방식
    - 읽기와 쓰기 작업에 대한 목록을 유지하고 완료되지 않은 트랜잭션의 스케줄에 따라 다른 트랜잭션의 수행을 제한한다.
    - (이게 비관적 동시성 제어에 해당하는게 맞는지 모르겠음. 애초에 무잠금은 낙관적에 해당하지 않나..)
    - 낙관적 -> 커밋 전에 충돌 여부 확인이고, 비관적 -> 트랜잭션 수행 중에 충돌 발생 가능성을 확인하고 계속 수행하거나 중단 또는 취소하는 것을 말하는듯.
- **잠금 기반 동시성 제어**
  - 비관적 동시성 제어의 한 종류다.
  - 경합 현상 및 확장성 문제가 발생할 수 있다는 단점이 있다. (교착상태)
   
### - 직렬화 가능성
- 트랜잭션은 데이터베이스의 상태를 읽고 쓰는 작업과 비즈니스 로직으로 구성된다. 데이터베이스의 관점에서 스케줄이란 트랜잭션을 수행하는 데 필요한 작업(읽기, 쓰기, 커밋, 중단 등)의 목록이다.
- (직렬화 가능성에 대해 설명해놨는데 정말 드럽게 설명을 못한다. 따로 검색해봐서 이해가 됐다.)
- **직렬 스케줄(Serial Schedule)**
  - 트랜잭션을 한 번에 하나씩 완전히 끝낸 후 다음 트랜잭션을 수행한다.
  - 직관적이지만 동시성이 없기 때문에 시스템의 처리량이 크게 제한되고 성능이 저하된다.
- **직렬화 가능한(serializable) 스케줄**
  - 동시에 실행된 어떤 스케줄이 해당 스케줄의 트랜잭션들을 순서대로 하나씩 실행한 결과와 같다면 그 스케줄은 직렬화 가능하다고 한다.
  - 예를들어 T1과 T2 수행에 대한 스케줄A(동시스케줄)가 다음과 같을 때 'T1일부 -> T2일부 -> T2일부 -> T1일부', 해당 스케줄의 결과가 직렬 스케줄 'T1 -> T2(혹은 T2 -> T1)'의 결과와 동일하다면 스케줄A를 직렬화 가능한 스케줄이라 한다. 

### - 트랜잭션 격리
- 격리 수준(isolation level)은 트랜잭션이 변경한 내용 중 어떤 부분이 언제 다른 트랜잭션에서 접근할 수 있는지를 정의한다.

### - 읽기 이상 현상
- **더티 읽기(dirty read)**
  - 아직 커밋되지 않은 다른 트랜잭션의 결과를 읽는 현상을 나타낸다.
- **반복 불가능 읽기(or 퍼지 읽기 fuzzy read)**
  - 트랜잭션이 동일한 로우를 두 번 쿼리했을 때 둘이 결과가 다른 현상을 나타낸다.
- **팬텀 읽기(phantom read)**
  - 여러 로우를 두 번 쿼리했을 때 결과가 다른 현상을 나타낸다.
  - 반복 불가능 읽기와 유사한 현상이지만 대상이 특정 레코드 대상 쿼리가 아닌 범위 쿼리이다.
 
### - 쓰기 이상 현상
- **갱신 분실**
  - 다른 트랜잭션이 같은 값을 동시에 수정할 때 발생한다. 나중에 커밋된 트랜잭션의 결과가 이전에 커밋된 트랜잭션의 결과를 덮어버리는 현상이다.
- **더티 쓰기**
  - 트랜잭션이 더티 읽기를 하고 수정 및 커밋하는 현상을 나타낸다. 즉, 커밋되지 않은 값을 읽고 값을 수정했기에 트랜잭션의 결과가 달라진다.
- **쓰기 치우침**
  - 개별 트랜잭션은 불변 조건을 충족하지만 동시 수행 시 조건이 위반되는 현상을 말한다.
  - 예를 들어 T1과 T2는 계좌 A1과 A2를 업데이트 한다. 각각 잔액은 1만원, 1만5천원이고 두 계좌의 합이 음수가 아니라면 각 계좌 잔액은 음수가 될 수 있다. T1과 T2는 2만을 인출하려고 하고, 인출하려는 당시에는 조건에 위반되지 않는다(2만5천 - 2만 >= 0). 하지만 수행 후 커밋된 결과는 조건을 위반한 상태가 된다.
 
### - 격리 수준
- 커밋 이전 읽기(read uncommitted)
  - 가장 낮은 격리 수준이다.
  - 이 수준에서 트랜잭션은 동시 수행 중인 다른 트랜잭션이 커밋하지 않은 데이터를 읽을 수 있다.(= 더티 읽기 허용)
- 직렬화 가능 수준
  - 가장 높은 격리 수준이다.
